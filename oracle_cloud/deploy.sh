...
echo ""
echo "[4/4] 서버 재시작 중..."
$SSH "$SERVER" 'cd ~/app && docker compose build --no-cache api && docker compose up -d api && docker compose stop nginx && docker compose rm -f nginx 2>/dev/null || true && docker compose up -d nginx && sleep 3 && docker compose ps'
