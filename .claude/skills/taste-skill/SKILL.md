...
description: Supanova Landing Page Design Engine. Generates premium, conversion-optimized landing pages using pure HTML + Tailwind CSS. Overrides default LLM biases toward generic templates. Enforces metric-based design rules, Korean typography standards, and hardware-accelerated motion for standalone HTML output. Supports offline environments without external internet access.
---
...
* **Offline-First Policy [CRITICAL]:** This project may run in environments WITHOUT external internet access. All resources MUST be self-contained:
  * **NO external CDN links** — Tailwind CSS, fonts, icons, animation libraries must all be inlined or bundled within the HTML file.
  * **NO external image URLs** — Use inline SVG, CSS gradients, or `data:` URIs for placeholder visuals instead of picsum.photos, Unsplash, or pravatar.cc.
  * When online access is available, CDN links MAY be used as an optional enhancement, but the page MUST function fully without them.
* **Styling:** Tailwind CSS — prefer the project's existing Tailwind setup. For standalone HTML files, inline critical Tailwind utility classes as `<style>` blocks rather than relying on `cdn.tailwindcss.com`. If CDN is acceptable (online environment), use `<script src="https://cdn.tailwindcss.com"></script>` as fallback. Use the `tailwind.config` script block for custom theme extensions (colors, fonts, spacing).
* **Typography — Korean First:**
  * **Primary Font:** `Pretendard` — load from local/self-hosted assets. If unavailable, use system font stack. CDN (`https://cdn.jsdelivr.net/gh/orioncactus/pretendard/dist/web/static/pretendard.min.css`) is optional fallback for online environments only.
  * **English Display Font:** Pair with `Geist`, `Outfit`, `Cabinet Grotesk`, or `Satoshi` for English headlines. Load from local assets or system fonts.
  * **Font Stack:** `font-family: 'Pretendard', 'Geist', -apple-system, BlinkMacSystemFont, system-ui, sans-serif;`
* **Icons:** Prefer inline SVG icons. Iconify with Solar icon set is allowed only in online environments. For offline, embed SVG directly: `<svg>...</svg>`.
* **Images:** Use inline SVG illustrations, CSS-generated shapes (`radial-gradient`, `conic-gradient`), or `data:image/svg+xml` URIs for all placeholder visuals. NEVER depend on external image services.
* **Animation Library:** Prefer pure CSS `@keyframes` and Tailwind's `animate-` utilities. For `MOTION_INTENSITY > 5`, use inline JavaScript with `IntersectionObserver` and CSS transitions. External libraries (Motion One, etc.) are optional for online environments only.
* **ANTI-EMOJI POLICY [CRITICAL]:** NEVER use emojis in markup or visible text content. Replace with inline SVG icons or clean SVG primitives.
* **Responsiveness:**
...
* **External Dependency Minimization:** Prefer zero external dependencies. If CDN is used (online only), total external scripts should not exceed 3.
...
### External Resources
* **NO external image URLs.** Use inline SVG, CSS gradients, or `data:` URIs exclusively.
* **NO mandatory CDN dependencies.** All resources must work offline. CDN links are optional enhancement only.
* **NO Unsplash, picsum.photos, pravatar.cc, or any external image service.**

## 8. THE SUPANOVA LANDING PAGE FORMULA
When generating a complete landing page, follow this exact structure:

### A. Document Setup (Offline-First)
```html
<!DOCTYPE html>
<html lang="ko">
<head>
  ...
  <!-- Tailwind: inline critical styles or use CDN as optional fallback -->
  <!-- <script src="https://cdn.tailwindcss.com"></script> -->
  <!-- Pretendard: load from local assets or use system font fallback -->
  <!-- <link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/orioncactus/pretendard/dist/web/static/pretendard.min.css"> -->
  <style>
    /* Inline critical Tailwind utilities and custom styles here */
    /* Font: Pretendard (local) → system-ui fallback */
    body { font-family: 'Pretendard', -apple-system, BlinkMacSystemFont, system-ui, sans-serif; }
  </style>
</head>
```
> **Note:** When online access is confirmed, uncomment CDN links for full feature support. The page MUST render correctly without them.
...
- [ ] Is Pretendard loaded (local/self-hosted) or system font fallback configured?
- [ ] Are all icons using inline SVG (or Iconify Solar set if online)?
- [ ] Does the page work fully WITHOUT external internet access?
- [ ] Are there zero external CDN dependencies that would break offline?
...
- [ ] Are there zero banned fonts, zero emoji, zero external image service links?
